document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function fetchSanctions(page) {
        try {
            $equipeId=$_Cookie('equipe_id')
            const response = await fetch(`http://127.0.0.1:8000/api/equipe/sanction?page=${page}`); 
      ...
      <div class="popup-card rounded-lg overflow-hidden w-full max-w-md mx-4 card-shadow">
        ...
        <div class="content-wrapper p-6 relative z-10">
          ...
          <div class="mt-6 flex justify-end">
            ...
            <button onClick='annulerSanction(${sanction.id})' class="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded shadow ${cacheDeleteButton}">
              Rejeter
            </button>
          </div>
        </div>
...
async function annulerSanction(sanctionId) {
    try {
        const response = await fetch(`http://127.0.0.1:8000/api/sanction/${sanctionId}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        if (!result || !response.ok) {
            console.error('Erreur lors de la suppression de la sanction', result);
           
        } else {
            console.log('Sanction supprimée avec succès', result);
            fetchSanctions();
            document.getElementById('sanctionContent').classList.add('hidden');
        }
    } catch (error) {
        console.error('Une exception s\'est produite :', error);
    }
}

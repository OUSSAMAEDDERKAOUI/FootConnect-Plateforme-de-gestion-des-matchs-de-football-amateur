...
document.getElementById('rapportForm').addEventListener('submit', async function (e) {
    ...
    const form = e.target;
    const game_id = form.game_id.value;
    console.log(game_id);
    ...
    try {
        const response = await fetch('http://127.0.0.1:8000/api/rapports', {
            method: 'POST',
            headers: {
    ...
    try {
        const response = await fetch(`http://127.0.0.1:8000/api/game/${game_id}/score`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
            },
            body: JSON.stringify(formScorData)
        });
...
.addEventListener("click", async (e) => {
    ...
    const team_id = equipe_exterieur; 
...
//     try {
//         window.open(`http://127.0.0.1:8000/api/rapports/${gameId}/pdf?token=${token}`);
//     } catch (error) {
